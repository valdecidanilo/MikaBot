...
// GPT-3
export const askOLD = async (input: string): Promise<string> => {
  text += "\nVocê: " + input;
...
export const ask = async (input: string): Promise<string> => {
  ...
  const response = await openai.createChatCompletion({
    model: "gpt-4-turbo",
    messages: [
      { role: "system", content: "Você é Mika, um assistente útil e amigável." },
      { role: "user", content: input },
    ],
    temperature: 0.02,
    max_tokens: 200,
  ...
  const result = response.data.choices[0].message?.content.trim() || "ops deu ruim!";
  text += "\nMika: " + result;
